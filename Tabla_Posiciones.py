...
# --- PANTALLA FINAL: INGRESO DE NOMBRE Y RANKING ---

def mostrar_fin_juego(pantalla, cola_eventos, datos_juego):
    """
    Muestra la pantalla final para ingresar el nombre, guardar el puntaje y ver el ranking.

    Args:
        pantalla (pygame.Surface): Superficie de destino.
        cola_eventos (list): Lista de eventos de pygame.
    ...
    nombre = ""
    escribiendo = True
    fuente = pygame.font.SysFont("Arial Black", 38)
    ...
    while escribiendo:
        ...
        if not ranking_mostrado:
            # Título
            titulo = fuente.render("¡Juego terminado!", True, color_boton)
            ...
            # Ranking
            titulo = fuente.render("TOP 10 PARTIDAS", True, color_boton)
            ...
            for i, partida in enumerate(partidas[:10]):
                ...
                texto_render = fuente_ranking.render(texto, True, color_texto)
            ...
            txt_volver = fuente_input.render("VOLVER", True, (255,255,255))
...
def mostrar_rankings(pantalla, cola_eventos):
    """
    Muestra el ranking de las mejores partidas y permite eliminar el top.
    ...
    Returns:
        str: Estado siguiente del flujo ("rankings", "menu" o "salir").
    """
    fuente_titulo = pygame.font.SysFont("Arial Black", 48)
    fuente_ranking = pygame.font.SysFont("Arial", 28)
    color_fondo = (230, 230, 255)
    color_boton = (200, 50, 50)
    color_boton_hover = (255, 80, 80)
    color_texto = (0, 0, 0)

    pantalla.fill(color_fondo)

    # Título
    titulo = fuente_titulo.render("TOP 10 PARTIDAS", True, (0, 120, 255))
    pantalla.blit(titulo, (pantalla.get_width() // 2 - titulo.get_width() // 2, 40))
    ...
    for i, partida in enumerate(partidas[:10]):
        texto = f"{i+1}. {partida['nombre']} - {partida['puntaje']} pts - {partida['fecha']}"
        texto_render = fuente_ranking.render(texto, True, color_texto)
        pantalla.blit(texto_render, (pantalla.get_width() // 2 - 320, 120 + i * 35))

    # Botón Eliminar Top
    boton_eliminar = pygame.Rect(pantalla.get_width() // 2 + 200, 500, 220, 50)
    mouse_pos = pygame.mouse.get_pos()
    color_btn = color_boton_hover if boton_eliminar.collidepoint(mouse_pos) else color_boton
    pygame.draw.rect(pantalla, color_btn, boton_eliminar, border_radius=15)
    pygame.draw.rect(pantalla, (255,255,255), boton_eliminar, 3, border_radius=15)
    fuente_btn = pygame.font.SysFont("Arial Black", 28)
    txt_btn = fuente_btn.render("ELIMINAR TOP", True, (255,255,255))
    pantalla.blit(txt_btn, txt_btn.get_rect(center=boton_eliminar.center))

    # Botón Volver
    boton_volver = pygame.Rect(pantalla.get_width() // 2 - 420, 500, 160, 50)
    pygame.draw.rect(pantalla, (0, 120, 255), boton_volver, border_radius=15)
    pygame.draw.rect(pantalla, (255,255,255), boton_volver, 3, border_radius=15)
    txt_volver = fuente_btn.render("VOLVER", True, (255,255,255))
    pantalla.blit(txt_volver, txt_volver.get_rect(center=boton_volver.center))
    ...
    for evento in cola_eventos:
        if evento.type == pygame.QUIT:
            ...
            if boton_volver.collidepoint(evento.pos):
                return "menu"
            elif boton_eliminar.collidepoint(evento.pos):
                # Elimina el archivo de ranking
                if os.path.exists("partidas.json"):
                    with open("partidas.json", "w", encoding="utf-8") as f:
                        f.write("[]")
                return "rankings"  # Refresca la pantalla
